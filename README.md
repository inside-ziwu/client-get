...
| 动态源管理 | ✅ | 行业动态的动态源监控页：种子定义的源列表只读展示 + 启停 + 上次成功时间 / 错误计数 + 「立即抓取」；每实例每天北京 08:00 自动抓取（`INDUSTRY_NEWS_FETCH_ENABLED`）。旧「情报源管理」页面与 `intelligence_*` 表已随 PR B 删除（迁移 20260824_0002）。`app/services/industry_news/`、`app/workers/industry_news_fetch.py` |
...
| 行业动态（阅读侧） | ✅ | 实例内按行业可见的每日动态流：标题 + 原文链接、来源 / 类别 / 语种筛选、「只看未读」、按用户已读、90 天窗口、同稿去重。tenant `/industry-news`（旧 `/intelligence` 页已随 PR B 删除） |
...
│   ├── alembic/        69 个迁移，仓库与生产 head=20260714_0001
│   └── scripts/        运维脚本（见 §9）
