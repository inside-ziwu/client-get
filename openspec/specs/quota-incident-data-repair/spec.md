# quota-incident-data-repair Specification

## Purpose

2026-07-02 配额级联事故受影响 enrollment 的一次性数据修复语义：修复集实时反查、备份→恢复→删除的原子执行、默认只读与二重确认、幂等。执行记录与背景见归档 change `restore-quota-incident-enrollments`。
