# sending-quota-circuit-breaker Specification

## Purpose

发送 worker 对 EngageLab 额度耗尽错误的识别（生产日志校准的关键词/错误码模式）、按域名熔断（当天停发、次日北京零点被动恢复）、配额错误的无失败记录处理（删行 defer + enrollment 推迟重建）与防误判护栏（瞬时限流走重试链、连续 defer 上限、未知 4xx 临时化）。历史决策与事故背景见归档 change `fix-quota-exhaustion-cascade`。
