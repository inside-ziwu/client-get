...
- 未识别国家不做笼统兜底：先通过映射表和 ISO 代码识别国家；仍无法识别时保留原始值，方便继续发现漏项并补充映射。
...
- 未识别国家是否需要 tooltip：本次先保留原始值，后续若需要可在确认具体漏项后补映射或增加排查辅助。
...
- 调整 `countryZh()` 识别链路：空值仍为 `-`；先查显式映射，再识别 ISO2/ISO3 代码；仍无法识别时返回原始值，方便继续补映射。
...
- Edge case: 输入未知三字母代码，保留原始值，便于继续发现并补充映射。
...
| Risk | Mitigation |
|------|------------|
| 国家映射不完整，后续仍出现代码 | 补齐主要国家 ISO3，并用 ISO 代码识别覆盖更多国家；未识别值保留原始值以便补表 |
| 原始值继续出现影响中文体验 | 把原始值视为映射漏项，后续按实际数据补充映射，而不是用笼统兜底掩盖 |
