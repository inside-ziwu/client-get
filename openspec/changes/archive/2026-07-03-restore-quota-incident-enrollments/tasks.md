...
- [x] 5.2 已归档(2026-07-03,specs 已同步主 spec)
...
- 补充清理（2026-07-03 08:52 UTC，用户触发）：修复集 enrollment 名下窗口外遗留 failed 邮件 54 行（52 行为 06-12~07-01 老重试模型历史噪音、1 行为 07-03 部署前重试耗尽、1 行为 07-03 部署后英国计划瞬时失败——非配额类，重试链正常继续），补入同一备份表后删除；备份表总行数 13,996，修复集相关 failed 残留 0
