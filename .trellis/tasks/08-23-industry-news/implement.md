...
| A8 | CLI `scripts/run_industry_news_fetch.py`（`--once` / `--source` / `--dry-run` / `--from-file`，照 `run_sending_worker.py` 模式） | CLI | 真站冒烟（信息性，不是合并门槛）：`--from-file app/data/industry_news_sources_pcb.json --dry-run`，记录每源条数与样本到 `research/live-fetch-<日期>.md`；据样本定稿 PCB Update 规则并回填种子与 fixture；个别源当日 0 条记为环境事项，不阻塞 | — |
