...
- 新增源时程序先做 RSS 自动发现；发现不了且仓库无其解析规则，则每轮 0 条并计入错误计数。

### R4 数据

- 动态源与动态均带 `instance_id`；动态继承其源的类别与语种；已读按（用户，动态）记录，未读即无记录。
- 90 天窗口按抓取时间计；更早的动态留库不展示，不做物理删除。
- 首批种子只导入 Instance A（`instance_id = default`），14 行见下表；Instance B 不导入。种子导入生产属写操作，按三段式逐次确认。
...
- [ ] AC1 Instance A 任一 PCB 租户用户登录后在「行业动态」看到 14 个源的动态，时间倒序、未读高亮；点击打开原文后该条变已读且只对该用户生效。
- [ ] AC2 类别 / 来源 / 语种筛选与「只看未读」可组合且结果正确；抓取时间早于 90 天的动态不出现。
- [ ] AC3 08:00 例行抓取后，当天新增动态出现在列表顶部；同稿在多个源出现只显示一条。
- [ ] AC4 管理端显示各源上次成功时间与错误计数；把某源地址改错后下一轮该源错误计数 +1、其余源正常；「立即抓取」可触发一轮；停用的源不再抓取。
- [ ] AC5 Instance B 租户打开「行业动态」看到空态提示，无报错。
- [ ] AC6 生产无 `intelligence_*` 表、端点、页面残留；`uv run pytest -q`、`pnpm type-check` 通过；去重与 90 天窗口在 Neon 开发库有断言记录。
