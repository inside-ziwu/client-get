...
- `audit_logs`、`emails`、`intelligence_articles` 按 `created_at` 月度 RANGE 分区，各有 DEFAULT 分区兜底；`app/db/partitions.py::ensure_partitions` 在启动时建当月与下月分区，`scripts/maintain_partitions.py` 可定时维护。
