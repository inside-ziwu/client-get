...
- 可空字段用 `?:`；枚举用字符串字面量联合（`IndustryNewsStrategy = 'rss' | 'html' | 'jsonld'`），与后端 `Literal` / CHECK 一致。
- 不用 `any`；后端 JSON 配置列用 `Record<string, unknown>`（如 `AiModel.config`、`Tenant.settings`），不用 `any` 兜底。
