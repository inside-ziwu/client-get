# instance-isolation Specification

## Purpose

同库多实例的账户体系与任务隔离:平台管理员、租户、平台配置、认证 token(JWT iid + 每实例独立密钥)与 Worker 任务按 instance_id 隔离,全局业务数据池共享。来源:归档 change multi-instance-deployment。
