    ...
    async def sync_platform_email_template(
        ...
        *,
        ...
        if not template["is_active"]:
            raise AppError(code="INVALID_STATE", message="只能同步已启用的模板", status_code=400)

        industry = template["industry"]

        result = await conn.execute(
            text("SELECT id FROM tenants WHERE industry = :industry AND status = 'active'"),
            {"industry": industry},
        )
        tenant_rows = result.mappings().all()
        ...
        existing_result = await conn.execute(
            text(
                """
                SELECT tenant_id FROM email_templates
                WHERE platform_template_id = :template_id
                  AND deleted_at IS NULL
                """
            ),
            {"template_id": template_id},
        )
        existing_tenant_ids = {str(row["tenant_id"]) for row in existing_result.mappings().all()}
        ...
        for tid in tenant_ids:
            ...
            await conn.execute(
                ...
                ),
                {
                    ...
                    "body_html": template["body_html"],
                    "body_text": template["body_text"],
                    "variables": self._to_json(template["variables"]),
                },
            )
            created.append(tid)
