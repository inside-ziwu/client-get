...
async def get_collection_task_monitor(context: PlatformAuthContext = Depends(get_current_platform_user)) -> dict:
    items = await service.get_collection_task_monitor(context.connection)
    return paginated_response(items, total=len(items))


@router.get("/data-sources")
async def list_data_sources(context: PlatformAuthContext = Depends(get_current_platform_user)) -> dict:
    items = await service.list_data_sources(context.connection)
    return paginated_response(items, total=len(items))


@router.post("/data-sources")
async def create_data_source(payload: dict, context: PlatformAuthContext = Depends(get_current_platform_user)) -> dict:
...
@router.patch("/data-sources/{source_type}")
async def patch_data_source(
    ...
    return success_response(
        await service.patch_data_source(
            context.connection,
...
@router.get("/data-sources/{source_type}/credentials")
async def list_data_source_credentials(
    source_type: str,
    context: PlatformAuthContext = Depends(get_current_platform_user),
) -> dict:
    items = await service.list_data_source_credentials(context.connection, source_type)
    return paginated_response(items, total=len(items))


@router.post("/data-sources/{source_type}/credentials")
async def create_data_source_credentials(
    source_type: str,
    payload: dict,
    context: PlatformAuthContext = Depends(get_current_platform_user),
) -> dict:
    return success_response(
        await service.create_data_source_credential(
            context.connection,
            source_type=source_type,
            payload=payload,
            platform_user_id=context.platform_user_id,
...
@router.patch("/data-sources/{source_type}/credentials/{credential_id}")
async def patch_data_source_credentials(
    source_type: str,
    credential_id: str,
    payload: dict,
    context: PlatformAuthContext = Depends(get_current_platform_user),
    ...
    return success_response(
        await service.patch_data_source_credential(
            context.connection,
            source_type=source_type,
...
@router.delete("/data-sources/{source_type}/credentials/{credential_id}")
async def delete_data_source_credentials(
    source_type: str,
    credential_id: str,
    context: PlatformAuthContext = Depends(get_current_platform_user),
) -> dict:
    await service.delete_data_source_credential(
        context.connection,
        source_type=source_type,
        credential_id=credential_id,
        platform_user_id=context.platform_user_id,
...
@router.patch("/data-sources/{source_type}/config")
async def patch_data_source_config(
    ...
    return success_response(
        await service.patch_data_source_config(
            context.connection,
            source_type=source_type,
            payload=payload,
            platform_user_id=context.platform_user_id,
