...
export default function TenantsPage() {
  ...
  const load = async () => {
    try {
      const response = await adminApi.tenants.list({ keyword: keyword || undefined });
  ...
  const loadWarmupRules = async () => {
    try {
      const response = await adminApi.warmupRules.get();
      const active = response.data.data.find((rule) => rule.is_active) ?? response.data.data[0] ?? null;
